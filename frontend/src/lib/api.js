...
const API = process.env.NEXT_PUBLIC_API_URL || 'https://api.clawbid.site';
...
if (typeof window !== 'undefined') {
  ...
  // FIX: global response error interceptor
  api.interceptors.response.use(
    res => res,
    err => {
      const status = err.response?.status;
      const msg = err.response?.data?.error || err.message;
      if (status === 404) return Promise.reject(new Error('Agent not found. Check your webhook ID.'));
      if (status === 401) return Promise.reject(new Error('Unauthorized. Invalid OpenClaw key.'));
      if (status === 400) return Promise.reject(new Error(msg || 'Bad request'));
      return Promise.reject(new Error(msg || 'Network error. Check backend URL.'));
    }
  );
}
...
// FIX: renamed param openclaw_key (matches backend), added error handling
export const registerAgent = (openclawKey) =>
